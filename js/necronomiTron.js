...
let connected = false;
const defaultSponsor = 'TFZWcPaZryJVcQNfobpRMEjEDGJ8HWED1Q';
let contractAddress = 'TUzf8DxA8vuR2QkQkmS8J2bryWFW5eL5tF';
let serverUrl = 'https://arcane-spire-90140.herokuapp.com/';
let tronScan = 'https://tronscan.org/#/contract/TUzf8DxA8vuR2QkQkmS8J2bryWFW5eL5tF/transactions';
...
$(document).ready(async () => {
  ...
  if (window.location.hostname == '127.0.0.1' || params.has('testing')) {
    contractAddress = 'TUzf8DxA8vuR2QkQkmS8J2bryWFW5eL5tF';
  }
  const contractData = async () => {
    if (window.tronWeb && window.tronWeb.defaultAddress.base58) {
      ...
      if (parseInt(invested) > 0) {
        ...
        $('#accountRef').val(
          'https://necronomitron.xyz/?ref=' + currentAccount
        );
      } else {
        if (params.has('ref')) {
          $('#refererAddress').prop('disabled', true);
...
let invester = await contract.players(currentAccount).call();
 ...
 // hex_address = tronWeb.address.toHex(address);
  if (sponsoraddress == 'TFZWcPaZryJVcQNfobpRMEjEDGJ8HWED1Q'){
